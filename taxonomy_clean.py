...
def draw_section(title, origin, cols, width):
    ...
    for i, row in subset.reset_index(drop=True).iterrows():
        ...
        label_text = wrap_label(code, width=13)
        label_y_center = (
            cy
            - fixed_radius
            - LABEL_GAP
...
def draw_svg_icon(ax, svg_path, x, y, size=0.49, color=icon_color):
    paths = load_svg_paths(svg_path)
...
# Matched visual style with Data and Visualization taxonomy figures
