...
def match_source_to_biblio(source_name, source_attrs, title_lookup):
    """Try to match a source to bibliography data by title, with improved matching strategies."""
    ...
    # Strategy 3: fuzzy match using similarity ratio (not just substring containment)
    best_match = None
...
def extract_data(
    ...
    def flatten_codes(codes):
        ...
        for code in as_list(codes):
            ...
            children = code.get("Code", [])
            flat.extend(flatten_codes(children))

        return flat


    code_guid_to_name = dict()
    all_codes = flatten_codes(project_json["Project"]["CodeBook"]["Codes"]["Code"])
